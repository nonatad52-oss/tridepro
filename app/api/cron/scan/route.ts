...
// 1. Identifica se a Bolsa do ativo está aberta neste exato minuto
function isMercadoAberto(ticker: string, dataHora: Date) {
  ...
  // Criptomoedas rodam 24/7
  if (ticker.endsWith('-USD')) return true;

  const isFimDeSemana = (dia === 0 || dia === 6);

  // Forex (ex: EURUSD=X) - 24/5 (Abre Dom 18h, Fecha Sex 17h)
  ...
  // Bolsa Brasileira B3 (ex: PETR4.SA) - Seg a Sex, 10h às 17h
  ...
  // Bolsa Americana NYSE/NASDAQ (ex: AAPL, TSLA) - Seg a Sex, 10h30 às 17h00
  if (isFimDeSemana) return false;
...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Filtro Inteligente de Horário de Mercado
    ...
    if (ativosAtivos.length === 0) {
       console.log("💤 Todas as bolsas dos ativos monitorados estão fechadas agora.");
    ...
    for (const ativo of ativosAtivos) {
      try {
        ...
        // Filtro Sniper: Só prossegue se houver extremidade no RSI ou um padrão de vela claro
...
3. Se os dados forem conflitantes (Ex: Tendência de Alta, mas Padrão de Baixa), o sinal deve ser "NEUTRO".
...
{"sinal": "COMPRA" | "VENDA" | "NEUTRO", "confianca_padrao": "XX%", "motivo": "Explicação técnica de no máximo 15 palavras."}`;
    ...
    if (torneioDeSinais.length > 0) {
      ...
      console.log("🔎 Varredura finalizada. O mercado não apresentou alinhamento perfeito (EMA + RSI + Gatilho).");
    }
